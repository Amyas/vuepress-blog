...
> 防抖是触发高频事件后 n 秒后函数执行一次，如果 n 秒内高频事件再次被触发，则重新计算时间
...
> 节流是触发高频事件后，n 秒内函数只会执行一次，会稀释函数的执行频率
        ...
        Array.from(nodes).forEach((node) => {
          ...
          if (node.hasAttribute("v-model")) {
            ...
            this._binding[attrVal]._directives.push(
              new Watcher("input", node, "value", this, attrVal)
            );
          ...
          if (tplReg.test(node.innerHTML)) {
            ...
            this._binding[attrVal]._directives.push(
              new Watcher("text", node, "innerHTML", this, attrVal)
            );
          }
        });
      }
